...
GRANT SELECT (id, name, slug, plan, status, logo_url, created_at, updated_at, limite_usuarios, ia_token_quota, ia_token_used, total_leads_mes, storage_used_bytes, storage_limit_bytes, settings) ON TABLE public.tenants TO authenticated;
...
RETURNS TABLE(has_token boolean, is_connected boolean, connected_phone text, connected_name text, webhook_registered boolean)
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  SELECT
    ...
    wc.connected_phone,
    wc.connected_name,
    COALESCE(wc.webhook_registered, false) AS webhook_registered
