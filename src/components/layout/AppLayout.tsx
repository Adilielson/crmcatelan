    ...
    <div className="flex h-screen bg-[#0E0E11]">
      <aside className="w-64 bg-[#0E0E11] border-r border-[#23232B] flex flex-col">
        <div className="p-6 border-b border-[#23232B]">
          <h1 className="text-xl font-bold text-white tracking-tight flex items-center gap-2 font-jakarta">
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link
                ...
                to={item.href}
                className={cn(
                  "flex items-center gap-3 px-3 py-2.5 rounded-[14px] text-sm transition-all duration-200 group",
                  isActive 
                    ? "bg-[#FFC400] text-[#1a1500] font-bold shadow-md shadow-[#FFC400]/10" 
                    : "text-slate-400 hover:bg-[#17171B] hover:text-[#FFC400]"
                )}
              >
                <item.icon className={cn(
                  "w-4 h-4 transition-colors",
                  isActive ? "text-[#1a1500]" : "text-slate-500 group-hover:text-[#FFC400]"
                )} />
                {item.label}
              </Link>
            );
        ...
        <div className="p-4 border-t border-[#23232B] bg-[#0E0E11]">
          <div className="flex items-center gap-3 mb-4 px-3 py-2 bg-[#17171B] rounded-[14px] border border-[#23232B]">
            <div className="w-9 h-9 rounded-full bg-[#FFC400] flex items-center justify-center text-[#1a1500] font-bold text-sm">
              {user.name.charAt(0)}
            </div>
          ...
          <button 
            onClick={logout}
            className="flex items-center gap-3 px-3 py-2.5 w-full text-xs font-bold text-[#D64545] hover:bg-red-500/10 rounded-[14px] transition-colors"
          >
      ...
      <main className="flex-1 overflow-auto flex flex-col bg-[#0E0E11]">
        <header className="h-16 bg-[#0E0E11] border-b border-[#23232B] flex items-center px-8 sticky top-0 z-10">
          <h2 className="text-sm font-bold text-slate-400 uppercase tracking-widest font-jakarta">
