...
const AppLayout = () => {
  ...
  // Itens da bottom nav mobile (5 principais)
  const bottomNav = [
  ...
  const currentLabel = menuItems.find(i => i.href === location.pathname)?.label || 'Painel Principal';

  return (
    <div className="flex h-screen bg-[#F6F7F9] relative z-10 text-ink font-jakarta">
      {/* Sidebar desktop */}
      <aside className="hidden md:flex w-64 bg-[#0E0E11] border-r border-[#23232B] flex-col shrink-0">
      ...
      {mobileMenuOpen && (
        <div className="md:hidden fixed inset-0 z-[60] flex">
          ...
          />
          <aside className="relative w-72 max-w-[85vw] bg-[#0E0E11] border-r border-[#23232B] flex flex-col shrink-0 animate-in slide-in-from-left">
            <div className="p-5 border-b border-[#23232B] flex items-start justify-between gap-3">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-[#FFC400]/10 rounded-xl">
                  <svg className="w-8 h-4 text-[#FFC400]" viewBox="0 0 60 28"><path d="M3 8 Q3 4 8 4 L24 4 Q28 4 28 9 L28 16 Q28 23 19 23 L11 23 Q3 23 3 14 Z M32 8 Q32 4 37 4 L53 4 Q57 4 57 9 L57 14 Q57 23 49 23 L41 23 Q32 23 32 16 Z M28 9 L32 9" fill="none" stroke="currentColor" strokeWidth="4" strokeLinejoin="round"/></svg>
                </div>
                <h1 className="text-[14px] font-black text-white tracking-[0.05em] leading-tight pt-1">ÓTICA CATELAN</h1>
              </div>
              <button
                onClick={() => setMobileMenuOpen(false)}
              ...
              {menuItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    to={item.href}
                    className={cn(
                      "flex items-center gap-3 px-4 py-3 rounded-[12px] text-[14px] font-semibold",
                      isActive
                        ? "bg-[#FFC400] text-[#1a1500]"
                        : "text-slate-300 hover:bg-[#17171B] hover:text-white"
                    )}
                  >
            ...
            <div className="p-4 border-t border-[#23232B]">
              <div className="flex items-center gap-3 mb-3 px-3 py-2.5 bg-[#17171B] rounded-[12px] border border-[#23232B]">
                <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[#FFC400] to-[#E0A500] flex items-center justify-center text-[#1a1500] font-black text-sm shrink-0">
                  {user.name.charAt(0)}
                </div>
              ...
              <button
                onClick={logout}
                className="flex items-center justify-center gap-2 px-4 py-3 w-full text-[11px] font-black text-[#D64545] hover:bg-[#D64545]/10 rounded-[12px] border border-[#D64545]/20"
              >
                <LogOut className="w-4 h-4" />
                ENCERRAR SESSÃO
              </button>
      ...
      <main className="flex-1 overflow-auto flex flex-col relative">
        <header className="h-14 md:h-20 bg-white/80 backdrop-blur-xl border-b border-[#E3E6EB] flex items-center px-4 md:px-10 sticky top-0 z-50 gap-3">
          {/* Botão menu mobile */}
          <button
            onClick={() => setMobileMenuOpen(true)}
            className="md:hidden p-2 -ml-2 rounded-lg text-ink hover:bg-[#F6F7F9]"
          ...
          <div className="flex flex-col min-w-0">
            <h2 className="hidden md:block text-[11px] font-black text-[#A7ADB8] uppercase tracking-[0.25em] leading-none mb-1">
              Plataforma Integrada
            </h2>
            <p className="text-[14px] md:text-[16px] font-black text-ink leading-none truncate">
              {currentLabel}
            </p>
          </div>
          <div className="ml-auto flex items-center gap-3 md:gap-6">
            <div className="nav-ai hidden lg:flex items-center gap-8 bg-[#F6F7F9] border border-[#E3E6EB] rounded-full px-5 py-2.5 transition-all hover:border-primary/30 group cursor-pointer">
            ...
            <div className="hidden lg:block h-8 w-[1px] bg-[#E3E6EB]" />
            <NotificationCenter />
          </div>
        </header>
        <div className="p-4 md:p-10 max-w-[1600px] mx-auto w-full flex-1 pb-24 md:pb-10 text-[14px] md:text-base">
          <Outlet />
        </div>
      </main>
      ...
      {/* Bottom nav mobile */}
      <nav
        className="md:hidden fixed bottom-0 inset-x-0 z-40 bg-[#0E0E11] border-t border-[#23232B] pb-[env(safe-area-inset-bottom)]"
      ...
      >
        <ul className="grid grid-cols-5">
            ...
            return (
              <li key={item.href}>
                <Link
                  to={item.href}
                  className={cn(
                    'flex flex-col items-center justify-center gap-0.5 py-2.5 text-[10px] font-semibold tracking-wide transition-colors',
                    isActive ? 'text-[#FFC400]' : 'text-slate-400 hover:text-white',
                  )}
                >
                  <Icon className={cn('h-5 w-5', isActive && 'drop-shadow-[0_0_6px_rgba(255,196,0,0.5)]')} />
          ...
          <li>
            <button
              onClick={() => setMobileMenuOpen(true)}
              className="w-full flex flex-col items-center justify-center gap-0.5 py-2.5 text-[10px] font-semibold tracking-wide text-slate-400 hover:text-white"
            >
